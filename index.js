...
const yt_api_key = ""; //youtube api
const prefix = "";//البرفكس
const color = "";//لون الكلام
client.login("");//التوكن
...
client.on("message", message => {
  ...
  if (message.content == prefix + "colors") {
    ...
    fs.readdir("./Data/colors", async (err, files) => {
      ...
      var image = await fsn.readFile(`./Data/colors/${f}`);
...
const replyMSG = JSON.parse(fs.readFileSync("./Data/replyMSG.json", "utf8"));

function saveReplay() {
  fs.writeFile("./Data/replyMSG.json", JSON.stringify(replyMSG), function(err) {
    if (err) throw err;
  });
}
...
const welcome = JSON.parse(fs.readFileSync("./Data/welcome.json", "utf8")); //ملف تخزين كود الويلكم
...
client.on("message", async message => {
  ...
  if (message.content.startsWith(prefix + "setWelcomer")) {
    if (!welcome[message.guild.id]) {
      ...
      fs.writeFile("./Data/welcome.json", JSON.stringify(welcome), err => {
        if (err) console.error(err);
      });
